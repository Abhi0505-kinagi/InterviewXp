...
function Card({ exp, interviewId }) {
  ...
  const [liked, setLiked] = useState(
    exp.upvotes.includes(localStorage.getItem("userId")),
  );
  const [disliked, setDisliked] = useState(
    exp.downvotes.includes(localStorage.getItem("userId")),
  );
  ...
  const handleFollow = async () => {
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/api/profile/${exp.userId?.name}/${endpoint}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const fetchAiSummary = async (text) => {
    try {
      ...
      const sentimentText =
        sentimentRes.prediction === 1 ? "Positive" : "Negative";
      const selectionText =
        selectionRes.prediction === 1
  ...
  return (
    <>
      <div className="posts-card fade-in" style={{ position: "relative" }}>
        <h3 style={{ fontFamily: "Times", color: "#869DAD" }}>
          {(exp.userId?.displayName).toUpperCase()}
        </h3>
        ...
        <h3 style={{ fontFamily: "Times", color: "#976de3" }}>
          Company: {exp.company}
        </h3>
        <br />
        <div style={{ fontFamily: "Times" }} className="meta">
          <strong style={{ fontFamily: "Times" }}>company Role:</strong>{" "}
          {exp.role}
          <br />
          <strong style={{ fontFamily: "Times" }}>candidate Level:</strong>{" "}
          {exp.experienceLevel}
          <br />
          <strong style={{ fontFamily: "Times" }}>
            Difficulty Level:
          </strong>{" "}
          ...
          <br />
          <strong style={{ fontFamily: "Times" }}>Posted on:</strong>{" "}
          ...
          <p>
            <strong style={{ fontFamily: "Times" }}>Interview Rounds:</strong>{" "}
            {exp.rounds.length}
          </p>
        ...
        <div className="overlay" onClick={() => setOpen(false)}>
          <div className="display-card" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="info">
              ...
              <strong className={`difficulty ${exp.difficulty.toLowerCase()}`}>
                {exp.difficulty}
              </strong>
            </p>
            ...
            <p className="note">
              <i>
                Note: We use pretrained ML to enhance content, but all summaries
                are reviewed by humans for accuracy and quality.
              </i>
            </p>
                ...
                >
                  ...
                  {comments.map((c) => (
                    <div key={c._id} style={{ marginBottom: "10px" }}>
                      <div style={{ display: "flex" }}>
                        {" "}
                        <strong style={{ fontFamily: "Times", color: "red" }}>
                          {c.userId?.name}
                        </strong>
                        <p
                          style={{
